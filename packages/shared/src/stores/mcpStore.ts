...
/** Aggregated tool-call stats for a single server, derived from the audit log. */
export interface McpCallSummary {
  totalCalls: number;
  blockedCalls: number;
  /** Distinct tool names called (most recent first, capped at 5). */
  recentTools: string[];
  /** Timestamp of the most recent call, or null if none. */
...
export async function scanServer(server: McpServer): Promise<McpScanResult> {
  ...
  // Fix B: If the scanner grades this server as danger, actually disable it so the
  // audit log entry below ("blocked") is semantically truthful.
  if (result.riskLevel === "danger") {
    await toggleServer(server.id, false).catch(
      (err) => console.error("[MCP] auto-disable failed:", err),
    );

    // Fix C: Surface the danger detection in the global security event log so the
    // Security page score reflects real MCP risk.
  ...
  // outcome now accurately reflects the server's actual state after the scan.
...
// ── Per-server call summary ────────────────────────────────────────────────

/**
 * Aggregate tool-call statistics for one server from the audit log.
 * Returns zero-state values when no entries exist so callers need no null checks.
 */
export async function getServerCallSummary(serverId: string): Promise<McpCallSummary> {
  const db = await getDb();

  // Total + blocked counts in one pass.
  ...
  // Most-recently-used distinct tool names (exclude the internal scan event).
