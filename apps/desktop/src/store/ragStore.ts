...
export async function buildRagContext(query: string, topK = 3): Promise<string> {
  ...
  // Bilingual instruction ensures the context preamble works for both
  // Chinese and English users regardless of the UI language setting.
  return (
    `<knowledge_base>\n${body}\n</knowledge_base>\n\n` +
    `Please prioritize the above knowledge base content when answering the question below. ` +
    `/ 请优先参考以上知识库内容来回答下方问题。\n\n`
  );
}
