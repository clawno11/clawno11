...
function RuleCard({
  ...
  // BUG-03: Sync local edit state when rule prop changes externally (e.g. after priority swap)
  useEffect(() => {
    if (!editing) {
      setName(rule.name);
  ...
  // BUG-14: Auto-dismiss confirm state after 3 s (mirrors HistorySidebar behaviour)
  const startConfirm = () => {
    if (confirmTimerRef.current) clearTimeout(confirmTimerRef.current);
    setConfirming(true);
          ...
          {editing ? (
            ...
            <>
              ...
              <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                ...
                {/* Use live instance name; flag orphaned rules */}
                {rule.instanceId && (() => {
...
function TestPanel({ rules, instances }: { rules: RoutingRule[]; instances: { id: string; name: string }[] }) {
  const { t } = useTranslation();
      ...
      {testInput.trim() && (
        <div className={`flex items-center gap-2 p-3 rounded-lg text-sm ${
          ...
          {matched && matchedInst ? (
            ...
            /* Rule matched but target instance was deleted */
            <>
...
function AddRuleForm({
  ...
  const handleAdd = () => {
    ...
    // BUG-12: Deduplicate keywords; BUG-06: compute nextPriority at submit time
    const keywords = [...new Set(kwRaw.split(",").map((s) => s.trim()).filter(Boolean))];
    if (keywords.length === 0) return;
    const rules = listRules();
    // BUG-13: Use reduce instead of Math.max(...spread) to avoid stack overflow on large arrays
    const nextPriority = rules.length > 0
      ? rules.reduce((max, r) => Math.max(max, r.priority), 0) + 10
      : 10;
    // BUG-09: instanceLabel removed — live name resolved at render time
...
export function RouterPage() {
  ...
  const handleMove = useCallback((id: string, dir: "up" | "down") => {
    // Read the latest sorted snapshot from localStorage to avoid stale-closure ordering issues.
            ...
            {instances.map((inst) => (
              <div key={inst.id} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 border border-border">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground font-mono truncate">{inst.httpUrl}</p>
                </div>
              </div>
            ))}
          </div>
